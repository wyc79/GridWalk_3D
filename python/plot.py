...
# pairs = pairs + [((first, ho), (second, ho)) for first, second in combinations(order, 2) for ho in hue_order]

# annotator = Annotator(ax=mid_plot.ax, data=df, pairs=pairs,
#                     x=x, hue=hue, y=y, 
#                     order=order, hue_order=hue_order)
...
mid_plot.savefig(f"out/mid_spine__{y}.tiff") 
